...
public class Resolver {
    ...
    private void resolve(Expr expr) {
        switch (expr) {
            ...
            case Expr.ObjectLiteral e -> resolveObjectLiteralExpression(e);
        }
    }
    ...
    private void resolveLetStatement(Stmt.Let stmt) {
        declare(stmt.variableName()); // We do not define it right away so we can't use its own nameToken in the
                                      // initializer
    ...
    private void resolveClassStatement(Stmt.Class stmt) {
        ...
        for (Stmt.Def method : stmt.methods()) {
            FunctionType declaration = method.name().lexeme().equals("constructor") ? FunctionType.INITIALIZER
                    : FunctionType.METHOD;
    ...
    private void resolveLookupExpression(Expr.Lookup expr) {
        if (lookup(expr.identifierToken().lexeme()) == Boolean.FALSE) {
            errorReporter.report("Variable nameToken can not be used in its initializer.",
                    expr.identifierToken().line());
        }
    ...
    private void resolveObjectLiteralExpression(Expr.ObjectLiteral expr) {
