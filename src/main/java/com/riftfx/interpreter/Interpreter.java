...
import com.riftfx.stdlib.core.StandardLibrary;
import com.riftfx.stdlib.types.NativeDictionary;
...
public class Interpreter {
    ...
    private Object evaluate(Expr expr) {
        return switch (expr) {
            ...
            case Expr.ObjectLiteral e -> evaluateObjectLiteralExpression(e);
        };
    }
        ...
        if (stmt.superclassLookupExpression() != null) {
            ...
            if (!(superclass instanceof Class)) {
                throw new RuntimeError("Superclass must be a class.",
                        stmt.superclassLookupExpression().identifierToken());
            }
        }
    ...
    private Object evaluateAssignmentExpression(Expr.Assignment expr) {
        ...
        if (expr.resolution().distance != null) {
            currentEnvironment.updateAt(expr.assigneeIdentifierToken().lexeme(), expr.resolution().distance,
                    valueToAssign);
        } else {
    ...
    private Object evaluateCallExpression(Expr.Call expr) {
        ...
        if (!callable.acceptsArity(expr.arguments().size())) {
            throw new RuntimeError("Invalid number of arguments. Expected " + callable.arity() + " but got "
                    + expr.arguments().size() + ".", expr.leftParenthesis());
        }
    ...
    private List<Object> resolveArguments(Callable callable, Expr.Call expr) {
        ...
        for (var i = 0; i < expr.arguments().size(); i++) {
            ...
            if (currentArgument.nameToken() != null) {
                ...
                if (targetIndex == -1) {
                    throw new RuntimeError("No parameter named '" + currentArgument.nameToken().lexeme() + "' found.",
                            currentArgument.nameToken());
                }
            } else {
                // 2. Positional Argument / Trailing Lambda
                var isTrailingLambda = (i == expr.arguments().size() - 1)
                        && (currentArgument.value() instanceof Expr.Lambda);
                ...
                if (isTrailingLambda) {
                    ...
                    if (namedArgumentSeen) {
                        throw new RuntimeError("Positional arguments can not appear after named arguments.",
                                expr.leftParenthesis());
                    }
            ...
            if (isFilled[targetIndex]) {
                var errorToken = currentArgument.nameToken() != null ? currentArgument.nameToken()
                        : expr.leftParenthesis();
                var collisionName = parameterNames.size() > targetIndex ? parameterNames.get(targetIndex)
                        : "index " + targetIndex;
    ...
    private Object evaluateSuperExpression(Expr.Super expr) {
        ...
        if (method == null) {
            throw new RuntimeError("Undefined method '" + expr.memberIdentifier().lexeme() + "'.",
                    expr.memberIdentifier());
        }
    ...
    private Object evaluateObjectLiteralExpression(Expr.ObjectLiteral expr) {
        Map<String, Object> map = new HashMap<>();
        for (Expr.Property property : expr.properties()) {
            map.put(property.name().lexeme(), evaluate(property.value()));
        }
        return new NativeDictionary(map);
    }
