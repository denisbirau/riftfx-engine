...
public class ExpressionParser {
    ...
    private void initializeRules() {
        ...
        mixed(TokenType.LEFT_BRACKET, this::parseArrayDefinitionExpression, this::parseSubscriptGetExpression,
                Precedence.CALL);
        mixed(TokenType.LEFT_BRACE, this::parseObjectLiteralExpression, this::parseOmittedParenthesesCall,
                Precedence.CALL);
    }
    ...
    private Expr parseObjectLiteralExpression() {
        List<Expr.Property> properties = new ArrayList<>();

        if (!stream.check(TokenType.RIGHT_BRACE)) {
            do {
                Token key = stream.consume(TokenType.IDENTIFIER, "Expect property name.");
                stream.consume(TokenType.COLON, "Expect ':' after property name.");
                Expr value = parseExpression();
                properties.add(new Expr.Property(key, value));
            } while (stream.match(TokenType.COMMA));
        }

        stream.consume(TokenType.RIGHT_BRACE, "Expect '}' after object properties.");
